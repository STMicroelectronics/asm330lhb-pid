...
/**
  ...
  *
  */
int32_t __weak asm330lhb_read_reg(const stmdev_ctx_t *ctx, uint8_t reg,
                                  uint8_t *data,
                                  uint16_t len)
{
  int32_t ret = 0;
...
/**
  ...
  *
  */
int32_t __weak asm330lhb_write_reg(const stmdev_ctx_t *ctx, uint8_t reg,
                                   uint8_t *data,
                                   uint16_t len)
{
  int32_t ret = 0;
...
{
  asm330lhb_ctrl1_xl_t ctrl1_xl = {0};
  int32_t ret = 0;

  ret = asm330lhb_read_reg(ctx, ASM330LHB_CTRL1_XL,
                           (uint8_t *)&ctrl1_xl, 1);
                                   ...
                                   asm330lhb_odr_xl_t val)
{
  ...
  asm330lhb_emb_fsm_enable_t fsm_enable = {0};
  asm330lhb_fsm_odr_t fsm_odr = {0};
  uint8_t mlc_enable = 0;
  asm330lhb_mlc_odr_t mlc_odr = {0};
  asm330lhb_ctrl1_xl_t ctrl1_xl = {0};
  int32_t ret = 0;
      ...
      if (ret == 0)
      {
        switch (fsm_odr)
        {
          case ASM330LHB_ODR_FSM_12Hz5:
            if (odr_xl == ASM330LHB_XL_ODR_OFF)
            {
          ...
          case ASM330LHB_ODR_FSM_26Hz:
            ...
            else if (odr_xl == ASM330LHB_XL_ODR_12Hz5)
            {
              odr_xl = ASM330LHB_XL_ODR_26Hz;
            }
            else
          ...
          case ASM330LHB_ODR_FSM_52Hz:
            ...
            else if (odr_xl == ASM330LHB_XL_ODR_26Hz)
            {
              odr_xl = ASM330LHB_XL_ODR_52Hz;
            }
            else
          ...
          case ASM330LHB_ODR_FSM_104Hz:
            ...
            else if (odr_xl == ASM330LHB_XL_ODR_52Hz)
            {
              odr_xl = ASM330LHB_XL_ODR_104Hz;
            }
            else
...
{
  asm330lhb_ctrl2_g_t ctrl2_g = {0};
  int32_t ret = 0;
  ret = asm330lhb_read_reg(ctx, ASM330LHB_CTRL2_G,
                           (uint8_t *)&ctrl2_g, 1);

  if (ret == 0)
  {
    ctrl2_g.fs_g = (uint8_t)(val & 0x0F);
    ret = asm330lhb_write_reg(ctx, ASM330LHB_CTRL2_G,
                              (uint8_t *)&ctrl2_g, 1);
  }
  ...
  if (ret == 0)
  {
         ...
         fsm_enable.fsm_enable_b.fsm16_en) == PROPERTY_ENABLE)
    {
      ...
      if (ret == 0)
      {
        ...
        {
          case ASM330LHB_ODR_FSM_12Hz5:
            if (odr_gy == ASM330LHB_GY_ODR_OFF)
            {
          ...
          case ASM330LHB_ODR_FSM_26Hz:
            ...
            else if (odr_gy == ASM330LHB_GY_ODR_12Hz5)
            {
              odr_gy = ASM330LHB_GY_ODR_26Hz;
            }
            else
          ...
          case ASM330LHB_ODR_FSM_52Hz:
            ...
            else if (odr_gy == ASM330LHB_GY_ODR_26Hz)
            {
              odr_gy = ASM330LHB_GY_ODR_52Hz;
            }
            else
          ...
          case ASM330LHB_ODR_FSM_104Hz:
            ...
            else if (odr_gy == ASM330LHB_GY_ODR_52Hz)
            {
              odr_gy = ASM330LHB_GY_ODR_104Hz;
            }
            else
...
int32_t asm330lhb_block_data_update_set(const stmdev_ctx_t *ctx,
                                        uint8_t val)
{
  asm330lhb_ctrl3_c_t ctrl3_c = {0};
  int32_t ret = {0};
  ret = asm330lhb_read_reg(ctx, ASM330LHB_CTRL3_C,
                           (uint8_t *)&ctrl3_c, 1);

  if (ret == 0)
  {
    ctrl3_c.bdu = (uint8_t)(val & 0x01);
    ret = asm330lhb_write_reg(ctx, ASM330LHB_CTRL3_C,
                              (uint8_t *)&ctrl3_c, 1);
  }
...
{
  asm330lhb_ctrl6_c_t ctrl6_c = {0};
  int32_t ret = {0};
  ret = asm330lhb_read_reg(ctx, ASM330LHB_CTRL6_C,
                           (uint8_t *)&ctrl6_c, 1);

  if (ret == 0)
  {
    ctrl6_c.usr_off_w = (uint8_t)(val & 0x01);
    ret = asm330lhb_write_reg(ctx, ASM330LHB_CTRL6_C,
                              (uint8_t *)&ctrl6_c, 1);
  }
...
{
  asm330lhb_ctrl7_g_t ctrl7_g = {0};
  int32_t ret = {0};
  ret = asm330lhb_read_reg(ctx, ASM330LHB_CTRL7_G,
                           (uint8_t *)&ctrl7_g, 1);

  if (ret == 0)
  {
    ctrl7_g.g_hm_mode = (uint8_t)(val & 0x01);
    ret = asm330lhb_write_reg(ctx, ASM330LHB_CTRL7_G,
                              (uint8_t *)&ctrl7_g, 1);
  }
                                  ...
                                  asm330lhb_all_sources_t *val)
{
  ...
  if (ret == 0)
  {
    ret = asm330lhb_read_reg(ctx, ASM330LHB_MLC_STATUS_MAINPAGE,
...
int32_t asm330lhb_xl_flag_data_ready_get(const stmdev_ctx_t *ctx,
                                         uint8_t *val)
{
  asm330lhb_status_reg_t status_reg = {0};
  int32_t ret = {0};
                                      ...
                                      int8_t *val)
{
  ...
  ret = asm330lhb_write_reg(ctx, ASM330LHB_X_OFS_USR, (uint8_t *)val, 1);
                                      ...
                                      int8_t *val)
{
  ...
  ret = asm330lhb_read_reg(ctx, ASM330LHB_X_OFS_USR, (uint8_t *)val, 1);
                                      ...
                                      int8_t *val)
{
  ...
  ret = asm330lhb_write_reg(ctx, ASM330LHB_Y_OFS_USR, (uint8_t *)val, 1);
                                      ...
                                      int8_t *val)
{
  ...
  ret = asm330lhb_read_reg(ctx, ASM330LHB_Y_OFS_USR, (uint8_t *)val, 1);
                                      ...
                                      int8_t *val)
{
  ...
  ret = asm330lhb_write_reg(ctx, ASM330LHB_Z_OFS_USR, (uint8_t *)val, 1);
                                      ...
                                      int8_t *val)
{
  ...
  ret = asm330lhb_read_reg(ctx, ASM330LHB_Z_OFS_USR, (uint8_t *)val, 1);
...
{
  asm330lhb_ctrl10_c_t ctrl10_c = {0};
  int32_t ret = {0};

  ret = asm330lhb_read_reg(ctx, ASM330LHB_CTRL10_C,
                           (uint8_t *)&ctrl10_c, 1);
...
{
  asm330lhb_ctrl5_c_t ctrl5_c = {0};
  int32_t ret = {0};

  ret = asm330lhb_read_reg(ctx, ASM330LHB_CTRL5_C,
                           (uint8_t *)&ctrl5_c, 1);

  if (ret == 0)
  {
    ctrl5_c.rounding = (uint8_t)(val & 0x03);
    ret = asm330lhb_write_reg(ctx, ASM330LHB_CTRL5_C,
                              (uint8_t *)&ctrl5_c, 1);
  }
...
/**
  ...
  *
  */
int32_t asm330lhb_temperature_raw_get(const stmdev_ctx_t *ctx,
                                      int16_t *val)
{
  uint8_t buff[2] = {0};
  int32_t ret = {0};
...
/**
  ...
  *
  */
int32_t asm330lhb_angular_rate_raw_get(const stmdev_ctx_t *ctx,
                                       int16_t *val)
{
  uint8_t buff[6] = {0};
  int32_t ret = {0};
...
/**
  ...
  *
  */
int32_t asm330lhb_acceleration_raw_get(const stmdev_ctx_t *ctx,
                                       int16_t *val)
{
  uint8_t buff[6] = {0};
  int32_t ret = {0};
...
/**
  ...
  *
  */
int32_t asm330lhb_odr_cal_reg_set(const stmdev_ctx_t *ctx, int8_t val)
{
  asm330lhb_internal_freq_fine_t internal_freq_fine = {0};
  int32_t ret = {0};
...
/**
  ...
  *
  */
int32_t asm330lhb_odr_cal_reg_get(const stmdev_ctx_t *ctx, int8_t *val)
{
  asm330lhb_internal_freq_fine_t internal_freq_fine = {0};
  int32_t ret = {0};
...
{
  asm330lhb_func_cfg_access_t func_cfg_access = {0};
  int32_t ret = {0};
  ...
  if (ret == 0)
  {
    ...
    func_cfg_access.reg_access = (uint8_t)(val & 0x01);
...
{
  asm330lhb_page_rw_t page_rw = {0};
  asm330lhb_page_sel_t page_sel = {0};
  asm330lhb_page_address_t page_address = {0};
  uint8_t msb = {0};
  uint8_t lsb = {0};
  int32_t ret = {0};
  uint8_t i = {0};
  ...
  /* set page write */
  ret = asm330lhb_read_reg(ctx, ASM330LHB_PAGE_RW,
                           (uint8_t *)&page_rw, 1);
  if (ret != 0)
  {
    goto exit;
  ...
  ret = asm330lhb_write_reg(ctx, ASM330LHB_PAGE_RW,
                            (uint8_t *)&page_rw, 1);
                                  ...
                                  uint8_t *val)
{
  ...
  /* read value */
  ret = asm330lhb_read_reg(ctx, ASM330LHB_PAGE_VALUE, val, 1);
  if (ret != 0)
  {
    goto exit;
  }
...
{
  asm330lhb_counter_bdr_reg1_t counter_bdr_reg1 = {0};
  int32_t ret = {0};
...
int32_t asm330lhb_device_id_get(const stmdev_ctx_t *ctx, uint8_t *val)
{
  ...
  ret = asm330lhb_read_reg(ctx, ASM330LHB_WHO_AM_I, val, 1);
...
{
  asm330lhb_ctrl4_c_t ctrl4_c = {0};
  int32_t ret = {0};

  ret = asm330lhb_read_reg(ctx, ASM330LHB_CTRL4_C,
                           (uint8_t *)&ctrl4_c, 1);

  if (ret == 0)
  {
    ctrl4_c.lpf1_sel_g = (uint8_t)(val & 0x01);
    ret = asm330lhb_write_reg(ctx, ASM330LHB_CTRL4_C,
                              (uint8_t *)&ctrl4_c, 1);
  }
...
{
  asm330lhb_ctrl8_xl_t ctrl8_xl = {0};
  int32_t ret = {0};

  ret = asm330lhb_read_reg(ctx, ASM330LHB_CTRL8_XL,
                           (uint8_t *)&ctrl8_xl, 1);
                                        ...
                                        asm330lhb_hp_slope_xl_en_t *val)
{
  ...
  else
  {
    ...
    {
      case 0x00:
        *val = is_low_pass == 1u ? ASM330LHB_LP_ODR_DIV_4 : ASM330LHB_SLOPE_ODR_DIV_4;
        break;

      case 0x01:
        *val = is_low_pass == 1u ? ASM330LHB_LP_ODR_DIV_10 : ASM330LHB_HP_ODR_DIV_10;
        break;

      case 0x02:
        *val = is_low_pass == 1u ? ASM330LHB_LP_ODR_DIV_20 : ASM330LHB_HP_ODR_DIV_20;
        break;

      case 0x03:
        *val = is_low_pass == 1u ? ASM330LHB_LP_ODR_DIV_45 : ASM330LHB_HP_ODR_DIV_45;
        break;

      case 0x04:
        *val = is_low_pass == 1u ? ASM330LHB_LP_ODR_DIV_100 : ASM330LHB_HP_ODR_DIV_100;
        break;

      case 0x05:
        *val = is_low_pass == 1u ? ASM330LHB_LP_ODR_DIV_200 : ASM330LHB_HP_ODR_DIV_200;
        break;

      case 0x06:
        *val = is_low_pass == 1u ? ASM330LHB_LP_ODR_DIV_400 : ASM330LHB_HP_ODR_DIV_400;
        break;
...
/**
  ...
  *
  */
int32_t asm330lhb_xl_fast_settling_set(const stmdev_ctx_t *ctx,
                                       uint8_t val)
{
  asm330lhb_ctrl8_xl_t ctrl8_xl = {0};
  int32_t ret = {0};
...
/**
  ...
  *
  */
int32_t asm330lhb_xl_fast_settling_get(const stmdev_ctx_t *ctx,
                                       uint8_t *val)
{
  asm330lhb_ctrl8_xl_t ctrl8_xl = {0};
  int32_t ret = {0};
...
{
  asm330lhb_int_cfg0_t int_cfg0 = {0};
  int32_t ret = {0};
...
/**
  * @defgroup   ASM330LHB_ main_serial_interface
...
{
  asm330lhb_pin_ctrl_t pin_ctrl = {0};
  int32_t ret = {0};

  ret = asm330lhb_read_reg(ctx, ASM330LHB_PIN_CTRL,
                           (uint8_t *)&pin_ctrl, 1);

  if (ret == 0)
  {
    pin_ctrl.sdo_pu_en = (uint8_t)(val & 0x01);
    ret = asm330lhb_write_reg(ctx, ASM330LHB_PIN_CTRL,
                              (uint8_t *)&pin_ctrl, 1);
  }
...
{
  asm330lhb_i3c_bus_avb_t i3c_bus_avb = {0};
  int32_t ret = {0};
  ...
  if (ret == 0)
  {
    i3c_bus_avb.pd_dis_int1 = (uint8_t)(val & 0x01);
...
{
  asm330lhb_ctrl9_xl_t ctrl9_xl = {0};
  asm330lhb_i3c_bus_avb_t i3c_bus_avb = {0};
  int32_t ret = {0};
  ...
  if (ret == 0)
  {
    ctrl9_xl.i3c_disable = (uint8_t)((val >> 7) & 0x01);
...
{
  asm330lhb_pin_int2_route_t pin_int2_route = {0};
  asm330lhb_int_cfg1_t int_cfg1 = {0};
  int32_t ret = {0};
...
{
  asm330lhb_pin_int1_route_t pin_int1_route = {0};
  asm330lhb_int_cfg1_t int_cfg1 = {0};
  int32_t ret = {0};
...
{
  asm330lhb_wake_up_dur_t wake_up_dur = {0};
  int32_t ret = {0};
...
{
  asm330lhb_wake_up_ths_t wake_up_ths = {0};
  int32_t ret = {0};
...
{
  asm330lhb_free_fall_t free_fall = {0};
  int32_t ret = {0};
...
{
  asm330lhb_fifo_ctrl1_t fifo_ctrl1 = {0};
  asm330lhb_fifo_ctrl2_t fifo_ctrl2 = {0};
  int32_t ret = {0};
...
{
  asm330lhb_fifo_ctrl3_t fifo_ctrl3 = {0};
  int32_t ret = {0};
...
{
  asm330lhb_fifo_ctrl4_t fifo_ctrl4 = {0};
  int32_t ret = {0};
...
int32_t asm330lhb_batch_counter_threshold_set(const stmdev_ctx_t *ctx,
                                              uint16_t val)
{
  asm330lhb_counter_bdr_reg2_t counter_bdr_reg1 = {0};
  asm330lhb_counter_bdr_reg2_t counter_bdr_reg2 = {0};
  int32_t ret = {0};
...
{
  uint8_t reg[2] = {0};
  const asm330lhb_fifo_status1_t *fifo_status1 = (asm330lhb_fifo_status1_t *)&reg[0];
  const asm330lhb_fifo_status2_t *fifo_status2 = (asm330lhb_fifo_status2_t *)&reg[1];
  int32_t ret = {0};
...
{
  asm330lhb_fifo_data_out_tag_t fifo_data_out_tag = {0};
  int32_t ret = {0};
                                   ...
                                   asm330lhb_den_lh_t val)
{
  ...
  ret = asm330lhb_read_reg(ctx, ASM330LHB_CTRL9_XL,
                           (uint8_t *)&ctrl9_xl, 1);
...
/**
  ...
  *
  */
int32_t asm330lhb_den_mark_axis_x_get(const stmdev_ctx_t *ctx,
                                      uint8_t *val)
{
  asm330lhb_ctrl9_xl_t ctrl9_xl = {0};
  int32_t ret = {0};
...
/**
  ...
  *
  */
int32_t asm330lhb_den_mark_axis_y_get(const stmdev_ctx_t *ctx,
                                      uint8_t *val)
{
  asm330lhb_ctrl9_xl_t ctrl9_xl = {0};
  int32_t ret = {0};
...
/**
  ...
  *
  */
int32_t asm330lhb_den_mark_axis_z_get(const stmdev_ctx_t *ctx,
                                      uint8_t *val)
{
  asm330lhb_ctrl9_xl_t ctrl9_xl = {0};
  int32_t ret = {0};
...
{
  asm330lhb_fsm_status_a_mainpage_t status_a = {0};
  asm330lhb_fsm_status_b_mainpage_t status_b = {0};
  int32_t ret = {0};
...
int32_t asm330lhb_long_cnt_flag_data_ready_get(const stmdev_ctx_t *ctx,
                                               uint8_t *val)
{
  asm330lhb_emb_func_status_t emb_func_status = {0};
  int32_t ret = {0};
...
int32_t asm330lhb_emb_func_clk_dis_set(const stmdev_ctx_t *ctx, uint8_t val)
{
  ...
  if (ret == 0)
  {
    ...
    page_sel.emb_func_clk_dis = (uint8_t)(val & 0x01);
    page_sel.not_used_01 = 1;
...
int32_t asm330lhb_emb_fsm_en_set(const stmdev_ctx_t *ctx, uint8_t val)
{
  ...
  if (ret == 0)
  {
    emb_func_en_b.fsm_en = (uint8_t)(val & 0x01);
...
{
  int32_t ret = {0};
  asm330lhb_emb_func_en_b_t emb_func_en_b = {0};
...
int32_t asm330lhb_long_cnt_set(const stmdev_ctx_t *ctx, uint16_t val)
{
  ...
  if (ret == 0)
  {
    ret = asm330lhb_write_reg(ctx, ASM330LHB_FSM_LONG_COUNTER_L, buff,
                              2);
  }
...
int32_t asm330lhb_long_cnt_get(const stmdev_ctx_t *ctx, uint16_t *val)
{
  ...
  if (ret == 0)
  {
    ret = asm330lhb_read_reg(ctx, ASM330LHB_FSM_LONG_COUNTER_L, buff,
                             2);
    if (ret == 0)
    {
      *val = (uint16_t)(((uint16_t)buff[1] << 8) + buff[0]);
    }
  }
...
{
  asm330lhb_fsm_long_counter_clear_t fsm_long_counter_clear = {0};
  int32_t ret = {0};
  ...
  if (ret == 0)
  {
    fsm_long_counter_clear.fsm_lc_clr = (uint8_t)(val & 0x03);
...
/**
  ...
  *
  */
int32_t asm330lhb_fsm_out_get(const stmdev_ctx_t *ctx,
                              uint8_t *val)
{
  int32_t ret = {0};
  ...
  if (ret == 0)
  {
    ret = asm330lhb_read_reg(ctx, ASM330LHB_FSM_OUTS1, val, 16);
  }
...
{
  asm330lhb_emb_func_odr_cfg_b_t emb_func_odr_cfg_b = {0};
  int32_t ret = {0};
  ...
  if (ret == 0)
  {
    ...
    emb_func_odr_cfg_b.fsm_odr = (uint8_t)(val & 0x03);
...
{
  asm330lhb_emb_func_init_b_t emb_func_init_b = {0};
  int32_t ret = {0};
  ...
  if (ret == 0)
  {
    emb_func_init_b.fsm_init = (uint8_t)(val & 0x01);
                                         ...
                                         uint16_t val)
{
  ...
  buff[1] = (uint8_t)(val / 256U);
  buff[0] = (uint8_t)(val - (buff[1] * 256U));
  ret = asm330lhb_ln_pg_write_byte(ctx, ASM330LHB_FSM_LC_TIMEOUT_L,
                                   &buff[0]);
                                         ...
                                         uint16_t *val)
{
  ...
  ret = asm330lhb_ln_pg_read_byte(ctx, ASM330LHB_FSM_LC_TIMEOUT_L,
                                  &buff[0]);
                                             ...
                                             uint8_t *val)
{
  ...
  ret = asm330lhb_ln_pg_write_byte(ctx, ASM330LHB_FSM_PROGRAMS, val);
                                             ...
                                             uint8_t *val)
{
  ...
  ret = asm330lhb_ln_pg_read_byte(ctx, ASM330LHB_FSM_PROGRAMS, val);
                                        ...
                                        uint16_t val)
{
  ...
  buff[1] = (uint8_t)(val / 256U);
  buff[0] = (uint8_t)(val - (buff[1] * 256U));
  ret = asm330lhb_ln_pg_write_byte(ctx, ASM330LHB_FSM_START_ADD_L,
                                   &buff[0]);

  if (ret == 0)
  {
    ret = asm330lhb_ln_pg_write_byte(ctx, ASM330LHB_FSM_START_ADD_H,
                                     &buff[1]);
  }
                                        ...
                                        uint16_t *val)
{
  ...
  ret = asm330lhb_ln_pg_read_byte(ctx, ASM330LHB_FSM_START_ADD_L,
                                  &buff[0]);

  if (ret == 0)
  {
    ret = asm330lhb_ln_pg_read_byte(ctx, ASM330LHB_FSM_START_ADD_H,
                                    &buff[1]);
    if (ret == 0)
    {
      *val = (uint16_t)(((uint16_t)buff[1] << 8) + buff[0]);
...
{
  asm330lhb_emb_func_en_b_t reg = {0};
  int32_t ret = {0};
...
{
  asm330lhb_emb_func_odr_cfg_c_t reg = {0};
  int32_t ret = {0};
...
int32_t asm330lhb_mlc_init_set(const stmdev_ctx_t *ctx, uint8_t val)
{
  ...
  if (ret == 0)
  {
    emb_func_init_b.mlc_init = (uint8_t)(val & 0x01);
    ret = asm330lhb_write_reg(ctx, ASM330LHB_EMB_FUNC_INIT_B,
                              (uint8_t *)&emb_func_init_b, 1);
  }
...
/**
  ...
  *
  */
int32_t asm330lhb_mlc_out_get(const stmdev_ctx_t *ctx, uint8_t *val)
{
  int32_t ret = {0};
  ...
  if (ret == 0)
  {
    ret = asm330lhb_read_reg(ctx, ASM330LHB_MLC0_SRC, val, 8);
  }
